...
// albums
AlbumController.$injector = [ '$sce', 'kptService', '$scope' ];
...
FinancialDetailController.$injector = [ '$sce', 'kptService', '$scope', '$q' ];
function FinancialDetailController($sce, kptService, $scope, $q) {
	...
	function getFinancialDetail() {
		kptService.getFinancialDetails().success(function(results) {
			...
			vm.summary_of_total_credit_debit = {};
			...
			angular.forEach(results.data, function(item, ind) {
					if(item.price != 0){
						...
						if(item.type === 'income'){
							...
							// for summary grouped bars chart
							if(typeof(vm.summary_of_total_credit_debit[start_year_month]) === 'undefined'){
								vm.summary_of_total_credit_debit[start_year_month] = {};
								vm.summary_of_total_credit_debit[start_year_month]['State'] = start_year_month;
								vm.summary_of_total_credit_debit[start_year_month]['收入'] = item.price;
								vm.summary_of_total_credit_debit[start_year_month]['支出'] = 0;
							}
							else{
								vm.summary_of_total_credit_debit[start_year_month]['收入'] += item.price;
							}
							...
							if(typeof(vm.summary_of_total_credit_debit[start_year_month]) === 'undefined'){
								vm.summary_of_total_credit_debit[start_year_month] = {};
								vm.summary_of_total_credit_debit[start_year_month]['State'] = start_year_month;
								vm.summary_of_total_credit_debit[start_year_month]['支出'] = item.price;
								vm.summary_of_total_credit_debit[start_year_month]['收入'] = 0;
							}
							else{
								vm.summary_of_total_credit_debit[start_year_month]['支出'] += item.price;
							}
			...
			angular.forEach(vm.credit_grouped_bars_chart, function(item_credit_detail, ind){
				...
				angular.forEach(vm.credit_accounts, function(item_account, ind){
					...
					if(typeof(vm.credit_sum[item_account]) === 'undefined'){
						vm.credit_sum[item_account] = item_credit_detail[item_account];
					}
					else{
						vm.credit_sum[item_account] += item_credit_detail[item_account];
					}
				...
				// console.log(JSON.stringify(item_credit_detail,2,2));
				
				financial_grouped_bars_chart_credit_data.push(item_credit_detail);
			});
			financial_grouped_bars_chart_credit_data.push(vm.credit_sum);
			...
			angular.forEach(vm.debit_grouped_bars_chart, function(item_debit_detail, ind){
				...
				angular.forEach(vm.debit_accounts, function(item_account, ind){
					...
					if(typeof(vm.debit_sum[item_account]) === 'undefined'){
						vm.debit_sum[item_account] = item_debit_detail[item_account];
					}
					else{
						vm.debit_sum[item_account] += item_debit_detail[item_account];
					}
			...
			financial_grouped_bars_chart_debit_data.push(vm.debit_sum);
			// end of debit
			...
			angular.forEach(vm.summary_of_total_credit_debit, function(item_summary, ind){
				...
				vm.credit_debit_sum['支出'] += item_summary['支出'];
				vm.credit_debit_sum['收支差異-收入減支出'] += item_summary['收支差異-收入減支出'];
			...
			// apply defer to handle data (temp)
